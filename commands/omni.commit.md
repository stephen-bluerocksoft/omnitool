---
description: Analyze changes and create logically grouped git commits following Conventional Commits
---
...
7. **Show the commit plan** to the user before executing. Format:
...
8. **Ask the user to confirm** before proceeding:
   - "Proceed with these N commit(s)? (yes/no/edit)"
   - If the user says "edit", ask what they want to change and revise the plan
   - If the user says "no", stop
   - If the user says "yes" or "proceed", continue to Phase 4

### Phase 4: Execute Commits

9. **Execute each commit sequentially**. For each logical group:
   ...
   b. Verify staging is correct:
   ```sh
   git diff --cached --stat
   ```

   c. Create the commit using a HEREDOC for proper formatting:
   ```sh
   ...
   d. Verify the commit succeeded:
   ```sh
   git log --oneline -1
   ```

10. **If a commit fails** (e.g., pre-commit hook rejection):
...
11. **Show final summary** after all commits:
